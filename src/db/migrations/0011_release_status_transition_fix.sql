-- Align release status transition trigger with migration 0004 lifecycle statuses.
-- Previous trigger enforced legacy values (validating, distributed, live, etc.) and
-- required INSERT status = draft, which blocked published catalog imports.
...
begin
  ...
  allowed := case
    ...
    when old.status = 'published' and new.status in ('scheduled', 'archived') then true
